...
public class MainActivity extends FragmentActivity implements FragmentCallback, ControllerCallback {
    ...
    public void fragmentCallback(int requestCode)
    {
        switch (requestCode)
        {
            ...
            {
                fragmentManager.beginTransaction().replace(R.id.container_left, mRecyclerFragment).addToBackStack("recycler").commit();
                break;
            }
    ...
    public void fragmentCallback(int requestCode, String city)
    {
        switch (requestCode)
        {
            case (RequestCode.CALL_WEATHER):
            {
                ...
                mWeatherFragment = new WeatherFragment();
                fragmentManager.beginTransaction().replace(R.id.container_left, mWeatherFragment).addToBackStack("weather").commit();
                break;
            }
